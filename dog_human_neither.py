...
if __name__ == "__main__":
    ...
    x_train = np.load("custom_images/balanced/x_train.npy")
    y_train = np.load("custom_images/balanced/y_train.npy")

    x_test = np.load("custom_images/balanced/x_test.npy")
    y_test = np.load("custom_images/balanced/y_test.npy")

    x_valid = np.load("custom_images/balanced/x_valid.npy")
    y_valid = np.load("custom_images/balanced/y_valid.npy")
    ...
    checkpointer = ModelCheckpoint(filepath='saved_models/weights.best.start_net_pre.hdf5',
    ...
    # Load weights
    model.load_weights('saved_models/weights.best.start_net_pre.hdf5')
    model.save("start_net_pre.h5")
    ...
    model.compile(loss='categorical_crossentropy', optimizer=optimizers.Adamax(1e-4), metrics=['accuracy'])

    checkpointer = ModelCheckpoint(filepath='saved_models/weights.best.start_net.hdf5',
                                   verbose=1, save_best_only=True)
    ...
    model.load_weights('saved_models/weights.best.start_net.hdf5')
    predictions = model.predict_generator(test_datagen.flow(x_test, shuffle=False))
    test_accuracy = 100 * np.sum(np.argmax(predictions, axis=1) == np.argmax(y_test, axis=1)) / len(predictions)
    print('Test accuracy: %.4f%%' % test_accuracy)

    model.save("start_net.h5")
